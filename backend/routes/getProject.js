...
router.get("/publisher/:id", async (req, res) => {
  ...
  if(project.applicants && project.applicants.length > 0){
    ...
    const applicants = await collection.find(query).toArray();
     ...
     //TODO: get ratings of each applicants (projects: finished & _id matches) 
    collection = await db.collection("projects");
      for (let applicant of applicants) {
        ...
        let completedProjects = await collection.find(completedProjectsQuery).toArray();

        let totalRating = 0;
        let ratingCount = 0;
        for (let project of completedProjects) {
          if (project.developer_rating) {
            totalRating += parseFloat(project.developer_rating);
            ratingCount += 1;
          }
        }

        applicant.averageRating = ratingCount > 0 ? (totalRating / ratingCount).toFixed(1) : null;
